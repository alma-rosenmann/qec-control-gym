...
class QECMaintenanceEnv(gym.Env):
    ...
    def step(self, action):
        self.step_count += 1
        prev_num_physical_errors = self.qubit_errors[:, 0].copy()
        if self.use_hook_errors:
        ...
        # Check the errors induced/corrected by the agent
        change_in_physical_errors = np.sum(self.qubit_errors[:, 0] - prev_num_physical_errors)
        
        if change_in_physical_errors > 0: 
            reward = 1.0 - 0.4 * change_in_physical_errors
        else:
            # Because the change will be negative, we can still do minus
            reward = 1.0 - 0.2 * change_in_physical_errors
        ...
        self.syndrome_buffer[-1] = self.latest_raw_syndrome
    
        terminated = False
        # Added truncated variable
        truncated = False
